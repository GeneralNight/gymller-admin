<template>
    <div>
        ...
        <b-form @submit.prevent="editWorker" id="formEditWorker">
...
methods: {
            ...
            await this.$api.getGymPositions(this.slug).then(res=> {
                if(res.data.msg=='success') {
                    if(res.data.data.length>0){
                        ...
                        this.alertRegister.text = "É preciso cadastrar no mínimo uma posição(nível) para editar um funcionário!"
    ...
    async loadWorker() {
        this.loader.text = "Carregando dados do funcionário..."
        this.loader.visible = true
        this.alertRegister.status = false
        this.$api.indexWorkers(this.slug,this.workerId).then(res => {
            if(res.data.msg == 'success') {
                ...
                switch (msgCode) {
                    ...
                    break;
                    case '002':
                        this.alertRegister.text = "Verifique o funcionário. É aconselhável voltar e selecionar o funcionário novamente!"
                        this.alertRegister.variant = "warning"                        
                    break;
    ...
    async editWorker() {        
        ...
        await this.$api.updateWorkers(this.slug , this.workerId , this.form).then(res=>{
            if(res.data.msg == 'success') {
                this.alertRegister.text = "Funcionário editado com sucesso!"
                ...
                switch (msgCode) {
                    ...
                    break;
                    case '002':
                        this.alertRegister.text = "Verifique o funcionário e tente novamente. É aconselhável voltar e selecionar o funcionário novamente!"
                        this.alertRegister.variant = "warning"
                    break;
                    case '003':
                        this.alertRegister.text = "Outro funcionário já está usando esse CPF!"
                        this.alertRegister.variant = "warning"
                    break;
                    case '004':
                        this.alertRegister.text = "Outro funcionário já está usando esse RG!"
                        this.alertRegister.variant = "warning"
                    break;
                    case '005':
                        this.alertRegister.text = "Outro funcionário já está usando esse usuário!"
                        this.alertRegister.variant = "warning"
                    break;
                    default:
